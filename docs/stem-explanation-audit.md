...
- 總題數:325
- calculation 題:19
- single_choice 含具體數字題:62
- 可獨立計算驗證的題目:19 題(覆蓋 19/19 = 100% 計算題)
...
全部 19 題 calculation(19 題 100% 覆蓋)的 answer 數值通過獨立計算驗證;explanation 已清除所有「以 case_X 為例」硬編碼數字洩漏。剩餘 7 件 P2 為 trap_type 標籤精度問題,不影響使用者體驗(answer 正確、distractor 合理),建議於後續 review 統一。
